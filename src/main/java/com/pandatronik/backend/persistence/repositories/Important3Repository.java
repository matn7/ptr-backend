...
public interface Important3Repository extends CrudRepository<Important3Entity, Long> {
    ...
    Optional<Important3Entity> findById(@Param("userId") long userId, @Param("id") Long id);
    ...
    Optional<Important3Entity> findByDate(@Param("userId") long userId, @Param("day") int day,
                                          @Param("month") int month, @Param("year") int year);

    @Query("SELECT i FROM Important3Entity i WHERE " +
            "MONTH(i.startDate) = :month AND YEAR(i.startDate) = :year AND i.userId = :userId")
    List<Important3Entity> findByDate(@Param("userId") long userId,
