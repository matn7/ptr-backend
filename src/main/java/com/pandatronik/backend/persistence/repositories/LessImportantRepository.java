...
public interface LessImportantRepository extends CrudRepository<LessImportantEntity, Long> {
    ...
    @Query("SELECT i FROM LessImportantEntity i WHERE i.userEntityId =:userEntityId AND i.id = :id")
    Optional<LessImportantEntity> findById(@Param("userEntityId") long userEntityId, @Param("id") Long id);
    ...
    Optional<LessImportantEntity> findByDate(@Param("userEntityId") long userEntityId, @Param("day") int day,
        @Param("month") int month, @Param("year") int year);

    @Query("SELECT i FROM LessImportantEntity i WHERE " +
            "MONTH(i.startDate) = :month AND YEAR(i.startDate) = :year AND i.userEntityId = :userEntityId")
    List<LessImportantEntity> findByDate(@Param("userEntityId") long userEntityId,
