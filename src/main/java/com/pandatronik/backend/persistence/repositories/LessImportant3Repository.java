...
public interface LessImportant3Repository extends CrudRepository<LessImportant3Entity, Long> {
    ...
    @Query("SELECT i FROM LessImportant3Entity i WHERE i.userEntityId =:userEntityId AND i.id = :id")
    Optional<LessImportant3Entity> findById(@Param("userEntityId") long userEntityId, @Param("id") Long id);
    ...
    Optional<LessImportant3Entity> findByDate(@Param("userEntityId") long userEntityId, @Param("day") int day,
                                              @Param("month") int month, @Param("year") int year);

    @Query("SELECT i FROM LessImportant3Entity i WHERE " +
            "MONTH(i.startDate) = :month AND YEAR(i.startDate) = :year AND i.userEntityId = :userEntityId")
    List<LessImportant3Entity> findByDate(@Param("userEntityId") long userEntityId,
