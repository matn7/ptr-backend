...
public interface LessImportant3Repository extends CrudRepository<LessImportant3Entity, Long>, EntityRepository<LessImportant3Entity> {
    ...
    @Query("SELECT i FROM LessImportant3Entity i WHERE i.userId = :userId")
    Optional<List<LessImportant3Entity>> findByUserId(@Param("userId") UserEntity userEntity);
    ...
    Optional<LessImportant3Entity> findByDate(@Param("userId") UserEntity userId, @Param("day") int day,
                                              @Param("month") int month, @Param("year") int year);

    @Query("SELECT i FROM LessImportant3Entity i WHERE " +
            "MONTH(i.startDate) = :month AND YEAR(i.startDate) = :year AND i.userId = :userId")
    List<LessImportant3Entity> findByDate(@Param("userId") UserEntity userId,
