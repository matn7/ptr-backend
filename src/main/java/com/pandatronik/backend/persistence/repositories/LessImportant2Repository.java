...
public interface LessImportant2Repository extends CrudRepository<LessImportant2Entity, Long> {
    ...
    @Query("SELECT i FROM LessImportant2Entity i WHERE i.userEntityId =:userEntityId AND i.id = :id")
    Optional<LessImportant2Entity> findById(@Param("userEntityId") long userEntityId, @Param("id") Long id);
    ...
    Optional<LessImportant2Entity> findByDate(@Param("userEntityId") long userEntityId, @Param("day") int day,
                                              @Param("month") int month, @Param("year") int year);

    @Query("SELECT i FROM LessImportant2Entity i WHERE " +
            "MONTH(i.startDate) = :month AND YEAR(i.startDate) = :year AND i.userEntityId = :userEntityId")
    List<LessImportant2Entity> findByDate(@Param("userEntityId") long userEntityId,
                                              @Param("year") int year, @Param("month") int month);

    // statistics
    @Query("SELECT made, COUNT(made) FROM LessImportant2Entity i WHERE YEAR(i.startDate) = :year " +
    ...
    @Query("SELECT MONTH(i.startDate), AVG(made) FROM LessImportant2Entity i WHERE YEAR(i.startDate) = :year " +
    ...
    @Query("SELECT made FROM LessImportant2Entity i WHERE i.startDate >= :startDate and i.startDate <= :endDate"
